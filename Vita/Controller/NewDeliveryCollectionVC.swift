...
class NewDeliveryCollectionVC: UICollectionViewController, UICollectionViewDelegateFlowLayout        {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        let firstRecord = NewOrder(number: "1", iconImage: "qrcodeIcon.png", title:"Medicines", subTitle: "0 Medicines", stepCompleted: false, qrScanned: [])
        let secondRecord = NewOrder(number: "2", iconImage: "ic_location.png", title:"Destination", subTitle: "Please Select Destination",stepCompleted: false, qrScanned: [])
        let thirdRecord = NewOrder(number: "3", iconImage: "ic_carrier.png", title:"Carrier", subTitle: "Please Select Carrier", stepCompleted: false, qrScanned: [])
    ...
    */
    ...
    @objc func handleCreate(){
      ...
      //  let medList = ["panadol"]
        ...
        let   values = ["userId": uid, "txnId": key, "name": name, "phone": phone, "address": address,"progress": "in transit", "medList" : array[0].qrScanned ] as [String : Any]
     ...
     //   alertTheUser(title: "About to Confirm transaction", message: "This transaction is about to be confirmed")
        ...
        let   values1 = ["medList" : array[0].qrScanned ] as [String : Any]
        //refDb = Database.database().reference().child("orders")
        refDb1 = Database.database().reference().child("orders").child("medList")
        //   alertTheUser(title: "About to Confirm transaction", message: "This transaction is about to be confirmed")
        refDb1.updateChildValues(values1, withCompletionBlock: { (error, ref) in
            if let error = error {
                print("Failed to update database values with error: ", error.localizedDescription)
                return
            }
    ...
    func configureNavigationBar(){
      ...
      //  navigationItem.title = "Prepare delivery"
       ...
       // navigationItem.rightBarButtonItem = UIBarButtonItem(image: UIImage(named: "ic_person_outline_white_2x.png")!.withRenderingMode(.alwaysOriginal), style: .plain, target: self, action: #selector(handleSignOut))
        ...
        navigationItem.leftBarButtonItem = UIBarButtonItem(image: #imageLiteral(resourceName: "ic_backwardArrow").withRenderingMode(.alwaysOriginal), style: .plain, target: self, action: #selector(handleBack))
